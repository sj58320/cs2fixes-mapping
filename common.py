...
def modify_gameinfo(gameinfo_path, core_gameinfo_path):
    ...
    # Accept quoted keys and whitespace variations, and avoid duplicate entries.
    content = ''.join(lines)
    existing = r'(?m)^\s*"?Game"?\s+"?csgo/addons/metamod"?\s*(?://[^\n]*)?$'
    if not re.search(existing, content):
        target = r'(?m)^([ \t]*)"?Game"?[ \t]+"?csgo"?[ \t]*(?://[^\n]*)?$'
        content, count = re.subn(target, lambda m: m[1] + 'Game\tcsgo/addons/metamod\n' + m[0], content, count=1)
        if count != 1:
            raise RuntimeError('Could not locate the csgo search path in gameinfo.gi.')
